...
def initialise():
    ...
    curr_dir = sys.path[0]
    overlay = Overlay(os.path.join(curr_dir, 'confidence_logits.bit'))
...
#     output_buffer.resize(output_size * 2)
    ...
    for i in range(10):
        ...
        dma_recv.transfer(output_buffer)
        dma_recv.wait()
        values.append(output_buffer[0])  # Assuming each buffer has a single integer

    print(values)  # This will print the list of integer values
    
    max_val_pred = generate_output(values)
    print(max_val_pred)
